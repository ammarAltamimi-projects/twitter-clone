...
const Post = ({ post }) => {
  ...
  // state to open/close comments modal
  const [showComments, setShowComments] = useState(false);
  const [comment, setComment] = useState("");

  if (isAuthLoading) return <LoadingSpinner size="sm" />;
  if (!post) return null;

  const userId = authUser?._id;
  const postOwner = post.user;
  const ownerId = postOwner?._id;

  const isLiked = userId ? (post.likes?.includes(userId) ?? false) : false;
  const isMyPost = userId && ownerId ? userId === ownerId : false;
      ...
      <div className="flex gap-2 items-start p-4 border-b border-gray-700">
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="flex flex-col gap-3 overflow-hidden">
            <span>{post.text}</span>
            {post.img && (
              <div className="w-full max-h-[500px] overflow-hidden rounded-lg border border-gray-700">
                <img src={post.img} className="w-full h-full object-cover" alt="Post media" />
              </div>
            )}
          </div>
          <div className="flex justify-center mt-3">
            <div className="flex items-center gap-20">
              ...
              <div className="flex gap-1 items-center cursor-pointer group" onClick={handleLikePost}>
                ...
                {!isLiked && !isLiking && <FaRegHeart className="w-4 h-4 text-slate-500 group-hover:text-pink-500" />}
      ...
      {showComments && (
        <dialog open className="modal">
          <div className="modal-box rounded border border-gray-600">
            ...
            <form onSubmit={handlePostComment} className="flex gap-2 items-center mt-4 border-t border-gray-600 pt-2">
              <textarea
